...
public class GameTexasHoldem implements Serializable {
	...
	public void showWinners(List<IPlayer> winners){
		...
		for (IPlayer player : winners) {
			System.out.println("\t\tWinner "+player.getName()+" cards: "+player.getCards()[0]+ " & " + player.getCards()[1]+" ==> "+player.getRankingEnum().toString());
		}
	}
